...
;;  let list-reserves-ratio [0.03 0.045 0.06]
  set CAR 0.1
  ;;let list-CAR [0.08]
  let list-mean-wp [10 20 25 30 40 60]
  let list-stdev-wp [5 15 20]
  let list-reserves-ratio [0.03 0.06];

  set n-simulations 5 ; was 10


  let iWP 0
  ;while [idx-simulations <= n-simulations] [
  while [iWP < length list-mean-wp] [
    ; MEAN LOOOOP

    let iSTD 0
    while [iSTD < length list-stdev-wp] [
    ;while [iWP < length list-mean-wp] [

      set mean-wp item iWP list-mean-wp
      ; STD LOOOOOP
      let iResRatio 0
      while [iResRatio < length list-reserves-ratio] [
      ;while[iSTD < length list-stdev-wp] [
        set stdev-wp item iSTD list-stdev-wp
        ...
        ;while [iResRatio < length list-reserves-ratio] [
          ...
          ;type " CAR: " type CAR
          type "mean-withdrawl-probability: " type mean-wp
          type "std-withdrawl-probability: " type stdev-wp
...
; SAVERRRRRRSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSS
...
;    setxy random-xcor random-ycor
    ...
    ;set withdraw-prob random wp ; WHERE WITHDRAWL PROBABILITY IS SET, was 21

    ; Sample the withdrawal probability from the normal distribution
    let sampled-withdraw-prob random-normal mean-wp stdev-wp

    ; Ensure the withdrawal probability is between 0 and 100
    set withdraw-prob max (list 0.01 min (list 99 sampled-withdraw-prob))
...
;; OUTPUTSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSS
...
to main-write-bank-ratios
  ...
  ask banks [
    ...
    let bank-number who
    matrix:set matrix-bank-ratios bank-number 0 mean-wp
    matrix:set matrix-bank-ratios bank-number 1 stdev-wp
    matrix:set matrix-bank-ratios bank-number 2 min-reserves-ratio
    ...
    matrix:set matrix-bank-ratios bank-number 3 bank-reserves
    matrix:set matrix-bank-ratios bank-number 4 bank-deposits
    matrix:set matrix-bank-ratios bank-number 5 equity
    ;matrix:set matrix-bank-ratios bank-number 5 total-assets
    ...
    matrix:set matrix-bank-ratios bank-number 6 credit-failure-indicator
    matrix:set matrix-bank-ratios bank-number 7 liquidity-failure-indicator

  ]
  ;; BANK RATIOSSSS OUTPUTSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSS
  file-open "bank-ratios.csv"
  file-type idx-simulations file-type " "
